...
if (loginForm) {
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // يمكن حفظ حالة "تذكرني" في localStorage إذا أردت
      if (remember) {
...
if (googleLogin) {
  googleLogin.addEventListener("click", async () => {
    ...
    try {
      ...
      // عند تسجيل الدخول الأول عبر Google، نتحقق من وجود المستخدم في Firestore
      ...
      if (!userDoc.exists()) {
        ...
        await setDoc(doc(db, "users", user.uid), {
          ...
          academyId: user.uid, // مؤقتاً
        });
        // إنشاء أكاديمية افتراضية
  ...
  registerForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        status: "active",
        createdAt: new Date().toISOString(),
        academyId: user.uid, // سنقوم بربط الأكاديمية بـ uid مؤقتاً
...
if (forgotForm) {
  forgotForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // يمكن توجيه المستخدم إلى صفحة تسجيل الدخول بعد فترة
      setTimeout(() => {
...
// ============================================================
// 5. تسجيل الخروج (من أي مكان في التطبيق)
// ============================================================
document.addEventListener("click", (e) => {
  ...
  if (target) {
    ...
    signOut(auth)
      .then(() => {
        // حذف أي بيانات مخزنة محلياً إذا أردت
        localStorage.removeItem("rememberMe");
        window.location.href = "/login";
      })
      .catch((error) => {
...
// ============================================================
// 6. مراقبة حالة المصادقة (onAuthStateChanged)
// ============================================================
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user && !isPublic) {
    // إذا لم يكن المستخدم مسجلاً وحاول دخول صفحة محمية
    window.location.href = "/login";
    return;
  }

  if (user && (currentPath === "/login" || currentPath === "/register" || currentPath === "/forgot-password")) {
    // إذا كان مسجلاً وحاول دخول صفحات المصادقة، نوجهه للوحة التحكم
    window.location.href = "/dashboard";
    return;
  }

  // إذا كان المستخدم مسجلاً، نقوم بتحميل بياناته وتحديث الواجهة
  if (user) {
    try {
      const userDoc = await getDoc(doc(db, "users", user.uid));
      if (userDoc.exists()) {
        const data = userDoc.data();
        // تحديث اسم المستخدم في العناصر التي تحمل id="user-name"
...
// ============================================================
// 7. دالة مساعدة للتحقق من الصلاحيات (اختياري)
// ============================================================
...
// ============================================================
// 8. دالة لتحديث بيانات المستخدم (تستخدم في ملف التعريف)
// ============================================================
